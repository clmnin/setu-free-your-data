...
async def fetch_signed_consent(consent_id):
...
async def fi_data_request(signed_consent, consent_id):
...
async def fi_data_fetch(session_id, encryption_private_key, key_material):
    privateKey = await read_private_key()
